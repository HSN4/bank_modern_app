...
const Footer = () => (
  <section className={`${styles.flexCenter} ${styles.paddingY} mt-16 flex-col`}>
              ...
              {footerLinks.links.map((link, index) => (
                <li
                  ...
                  className={` font-poppins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${
                    index === footerLinks.links.length - 1 ? "mb-0" : "mb-3"
                  }`}
                >
                  {link.name}
                </li>
    ...
    <div className="flex justify-between items-center w-full md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3F3E45]">
      <p className="font-popping font-normal text-[18px] leading-[27px] text-white text-center">
      ...
      <div className="flex flex-row md:mt-0 mt-6">
        {socialMedia.map((socialMedia, index) => (
          <img
            ...
            key={socialMedia.id}
            className={`${
              index === socialMedia.length - 1 ? "mr-" : "mr-6"
